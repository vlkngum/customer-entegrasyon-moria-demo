...
// Background colors for different ERP platforms
const getErpPlatformBg = (platformName: string): string => {
  const bgColors: Record<string, string> = {
    'Asb Yazılım': 'bg-[#F2993A]',
    'Dia': 'bg-[#223A6A]',
    'Sysmond': 'bg-[#3B7C99]',
    'Paraşüt': 'bg-[#FF6F5B]',
    'Bizim Hesap': 'bg-[#2B5C54]',
    'Logo': 'bg-[#D13B3B]',
    'Logo İşbaşı': 'bg-[#D13B3B]',
    'IdeaConnect': 'bg-[#B6E07B]',
    'Nebim': 'bg-[#3B7C99]',
    'Eta': 'bg-[#223A6A]',
  };
  return bgColors[platformName] || 'bg-[#F2993A]';
};
...
export default function ErpSettingsPage() {
  ...
  const { fetchIntegrations, syncIntegrationsToRedux } = useIntegrations();
  ...
  // Fetch templates only once on mount
  useEffect(() => {
  ...
  // Fetch integrations only when accessToken changes
  useEffect(() => {
    if (extendedSession && extendedSession.accessToken) {
      loadIntegrations();
    }
  ...
  const handleCardClick = (template: ServiceTemplate) => {
    ...
    router.push(`/settings/erp/${routeName}`);
  };
  ...
  // Filter templates to show only ERP platforms
  // For now, show all templates since ERP type is not supported yet
  // TODO: Update when backend supports ERP type
  const erpTemplates = allTemplates.filter((template: ServiceTemplate) => 
    template.type === 'erp' // Temporarily use ecommerce type
  );
  ...
  return (
    <div className="px-6 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {erpTemplates.map((template: ServiceTemplate) => {
          const isConnected = isPlatformConnected(template);
          const bgColor = getErpPlatformBg(template.name);

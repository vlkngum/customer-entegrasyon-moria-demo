...
import { fetchServiceTemplates, selectEcommerceTemplates, selectIsLoading, selectError, clearTemplates } from '@/store/slices/ecommerceSlice';
...
// Background colors for different platforms
const getPlatformBg = (platformName: string): string => {
  const bgColors: Record<string, string> = {
    'İdeasoft': 'bg-[#99cf65]',
    'ankaEticaret': 'bg-[#223A6A]',
    'Opencart': 'bg-[#3B7C99]',
    'Woocommerce': 'bg-[#FF6F5B]',
    'Shopify': 'bg-[#2B5C54]',
    'Ticimax': 'bg-[#D13B3B]',
    'Wix': 'bg-[#D13B3B]',
    'Kolay Sipariş': 'bg-[#B6E07B]',
    'ETicaretSoft': 'bg-[#3B7C99]',
    'Shopier': 'bg-[#223A6A]',
    'İkas': 'bg-[#223A6A]',
    'Ethica': 'bg-[#4A90E2]',
    'Magento': 'bg-[#F46F25]',
    'ShopPHP': 'bg-[#6C5CE7]',
    'Platin Market': 'bg-[#00B894]',
    'T-Soft': 'bg-[#E17055]',
    'PrestaShop': 'bg-[#00D2D3]',
  };
...
export default function ErpSettingsPage() {
  ...
  // Redux selectors
  const ecommerceTemplates = useSelector(selectEcommerceTemplates);
  ...
  }, [session?.accessToken, fetchIntegrations, syncIntegrationsToRedux]);
  ...
  }, [dispatch, session?.accessToken, loadIntegrations]);
    ...
    <div className="px-6 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {ecommerceTemplates.map((template) => {
          ...
          const bgColor = getPlatformBg(template.name);

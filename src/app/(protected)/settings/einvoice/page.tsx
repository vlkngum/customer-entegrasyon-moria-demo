...
// Background colors for different invoice platforms
const getInvoicePlatformBg = (platformName: string): string => {
  const bgColors: Record<string, string> = {
    'Trendyol E-faturam (Sovos)': 'bg-[#b4c42c]',
    'Turkcell E-Fatura': 'bg-[#f7e5a6]',
    'Logo E-Fatura': 'bg-[#2b378a]',
    'GİB E-Arşiv': 'bg-[#d8232a]',
    'QNB eFinans E-Fatura': 'bg-[#b14f88]',
  };
  return bgColors[platformName] || 'bg-[#b4c42c]';
};
...
export default function EinvoiceSettingsPage() {
  ...
  const handleCardClick = (template: ServiceTemplate) => {
    ...
    router.push(`/settings/einvoice/${routeName}`);
  };
  ...
  // Filter templates to show only invoice platforms
  const invoiceTemplates = allTemplates.filter((template: ServiceTemplate) => 
  ...
  return (
    <div className="px-6 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {invoiceTemplates.map((template: ServiceTemplate) => {
          const isConnected = isPlatformConnected(template);
          const bgColor = getInvoicePlatformBg(template.name);
          ...
          return (
            ...
            >
              ...
              {/* Alt başlık ve dişli */}
              <div className="bg-[#F8FAFC] flex gap-2 items-center justify-center border-t border-gray-200 px-6 py-4 mt-auto w-full rounded-b-md">
                <FiSettings className="text-gray-500 self-center" size={20} />
                <span className="flex flex-col justify-center text-base font-medium text-gray-700 break-words text-center">{template.name} Ayarları</span>
              </div>
            </div>
          );
        })}
